...
const EditExam = () => {
  ...
  const parseExcelToQuestions = async (workbook) => {
    ...
    for (const ws of workbook.worksheets) {
      ...
      if (ws.getImages && ws.getImages().length > 0) {
        throw new Error(`File chứa hình ảnh trong sheet "${ws.name}". Vui lòng xóa và thử lại.`);
      }
      // 2. Kiểm tra drawings
      if (ws.drawings && ws.drawings.length > 0) {
        throw new Error(`File chứa hình vẽ/biểu đồ trong sheet "${ws.name}". Vui lòng xóa và thử lại.`);
      }
    }
    // 3. Kiểm tra workbook-level media
    if (workbook.media && workbook.media.length > 0) {
      throw new Error(`File chứa ${workbook.media.length} file media. Vui lòng xóa và thử lại.`);
    }
    // 4. Kiểm tra VBA/Macros
    if (workbook.vbaProject) {
      throw new Error(`File chứa VBA/Macros. Vui lòng lưu dạng .xlsx và thử lại.`);
    }

    // Convert ExcelJS worksheet to array of arrays (like XLSX.utils.sheet_to_json with header:1)
    const rows = [];
    sheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
      const rowData = [];
      row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
        // Security: Only accept primitive values, reject objects/formulas
        ...
        if (value && typeof value === 'object') {
          ...
          if (value.richText) {
            ...
            value = value.text; // Hyperlink text
          } else if (value instanceof Date) {
            value = value.toISOString();
          } else {
            value = ''; // Reject unknown objects for security
          }
        }
  ...
  const handleAnalyzeFile = async () => {
    ...
    try {
      ...
      const fileQuestions = await parseExcelToQuestions(workbook);
            ...
            {mcqQuestions.map((q, idx) => (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <button
                    onClick={() => {
                      setConfirmModal({ isOpen: true, questionId: q.id });
                    }}
        ...
        onConfirm={() => {
          setEditingExam({
            ...
            questions: editingExam.questions.filter(
              (x) => x.id !== confirmModal.questionId
            ),
          });
