    ...
    <div className="p-4">
      ...
      <div className="mx-auto w-full">
        <section className="w-full bg-white backdrop-blur border border-slate-200 rounded-2xl p-4 shadow-sm">
          <h2 className="text-sm font-semibold text-slate-700 mb-3">{t('my_exams', 'Đề thi của tôi', 'My Exams')}</h2>
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <input type="text" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm tên đề thi..." className="px-3 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-300" />
          ...
          {loading ? (
            ...
            <ul className="space-y-3">
              {filtered.map((e) => (
                <li key={e.id}>
                  <button onClick={() => openPreview(e.id)} className="w-full text-left px-4 py-3 rounded-xl bg-white shadow-md hover:shadow-lg transition-all duration-300 hover:ring-2 hover:ring-sky-400">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="font-semibold text-slate-800">{e.title || `Exam #${e.id}`}</div>
                        <div className="text-xs text-slate-500 mt-0.5 flex items-center gap-2">
                          <span>Trạng thái:</span>
                          <span className={`px-2 py-0.5 rounded ${e.status === 'published' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{e.status || 'draft'}</span>
                          {/* Pha thi theo time_open/time_close */}
                          {(() => { const p = getPhase(e); return (<span className={`px-2 py-0.5 rounded ${p.cls}`}>{p.label}</span>); })()}
                        </div>
                        <div className="mt-1 text-[11px] text-slate-500 flex items-center gap-2">
                          {e.time_open && <span>Open: {new Date(e.time_open).toLocaleString()}</span>}
                          {e.time_close && <span>Close: {new Date(e.time_close).toLocaleString()}</span>}
                        </div>
                      </div>
