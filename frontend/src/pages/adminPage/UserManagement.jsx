...
const UserManagement = () => {
    ...
    const getRoleBadge = (role) => {
        const config = {
            admin: "bg-red-500/10 text-red-500 border-red-500/20 shadow-sm transition-all duration-200",
            instructor: "bg-purple-500/10 text-purple-500 border-purple-500/20 shadow-sm transition-all duration-200",
            student: "bg-blue-500/10 text-blue-500 border-blue-500/20 shadow-sm transition-all duration-200",
            default: "bg-gray-500/10 text-gray-400 border-gray-500/20 shadow-sm transition-all duration-200"
        };

        const color = config[role?.toLowerCase()] || config.default;

        return (
            <span className={`inline-flex items-center justify-center px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-bold border whitespace-nowrap ${color}`}>
                {(role || 'N/A').toUpperCase()}
            </span>
        );
    };
                        ...
                        <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-lg mx-4">
                            ...
                            <div className="p-6 space-y-4">
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-medium text-white">{selectedUser.full_name || t('notUpdated')}</h3>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="text-xs text-gray-300 uppercase">{t('gender')}</label>
                                        <p className="text-white mt-1">{selectedUser.gender || t('notUpdated')}</p>
                                    </div>
                                    <div>
                                        <label className="text-xs text-gray-300 uppercase">{t('phoneNumber')}</label>
                                        <p className="text-white mt-1">{selectedUser.phone_number || t('notUpdated')}</p>
                                    </div>
                                    <div>
                                        <label className="text-xs text-gray-300 uppercase">{t('dateCreated')}</label>
                                        <p className="text-white mt-1">{new Date(selectedUser.created_at).toLocaleDateString(language === 'vi' ? 'vi-VN' : 'en-US')}</p>
                                    </div>
                                </div>

                                <div>
                                    <label className="text-xs text-gray-300 uppercase">{t('address')}</label>
                                    <p className="text-white mt-1">{selectedUser.address || t('notUpdated')}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="p-6 space-y-4">
                                ...
                                <div>
                                    ...
                                    {selectedUser.role === 'admin' && (
                                        <p className="text-xs text-yellow-400 mt-1">{t('adminRoleRestriction')}</p>
                                    )}
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm text-gray-300 mb-1">{t('phoneNumber')}</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-sm text-gray-300 mb-1">{t('gender')}</label>
                                        <select
                                            value={editForm.gender}
                                            onChange={(e) => setEditForm({ ...editForm, gender: e.target.value })}
                                            className="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-blue-500"
                                        >
                                            <option value="">{t('notUpdated')}</option>
                                ...
                                <div>
                                    <label className="block text-sm text-gray-300 mb-1">{t('address')}</label>
                                    <input
                                        type="text"
                        ...
                        <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-md mx-4">
                            <div className="p-6 text-center">
                                ...
                                <p className="text-gray-300 mb-6">
                                    {t('deleteConfirmText')} <span className="text-white font-medium">{selectedUser.email}</span>?
                                    {t('cannotUndo')}
                                </p>
