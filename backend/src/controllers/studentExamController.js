...
async function startExam(req, res) {
  try {
    ...
    const [subRows] = await sequelize.query(sel, {
      replacements: [submissionId, userId],
    });
    ...
    try {
      ...
      if (row && sub.status === "pending") {
        if (
          Number(row.face_verified) !== 1 ||
          Number(row.card_verified) !== 1 ||
          Number(row.monitor_agreed) !== 1
        ) {
          return res
            .status(400)
            .json({ message: "Chưa hoàn tất xác minh (ảnh/giám sát)" });
        }
      }
    } catch (e) {
      /* ignore if columns missing */
