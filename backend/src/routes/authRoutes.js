  ...
  router.post("/send-otp", async (req, res) => {
    try {
      ...
      if (!emailResult.success) {
        console.error(`[Send OTP] ❌ Lỗi gửi email đến ${email}:`, emailResult.error);
  ...
  router.post("/register", async (req, res) => {
    try {
      ...
      if (!otpData || !otpData.verified) {
        ...
        return res
          .status(400)
          .json({ message: "Email chưa được xác minh. Vui lòng xác minh email trước khi đăng ký", status: "error" });
      }
